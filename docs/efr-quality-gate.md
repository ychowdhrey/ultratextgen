...
widened** (§9.11): Updates read 8 PASS / 1 REVIEW / 2 FAIL (mean 3.61, median
2.8), Guides 19 / 1 / 12 (mean 6.97, median 5.5). The widening was aimed at
...
`discord-where-fonts-work` (5.6 → 9.5), `instagram-font-ideas` (6.8 → 10.7) and
`bio-formatting-without-spam` (2.6 → 7.1) crossed out of PASS with no word
changed. Site-wide, against the baseline main carried after its own em-dash template
fix, 629 pages improved, 2,870 held and 1,130 worsened by 0.5 or more (the
largest by 11.8, `library/devil-emoji`); the site median moved 10.0 → 10.5. **A widening is a re-baseline event**: the baseline was regenerated
...
rhetorical questions. Those were removed first (18.0 → 11.5 and 14.6 → 14.1),
...
11. **Widening the fact vocabulary moves pages nobody touched.**
    ...
    2026-09-02 (§4): three untouched guides crossed out of PASS, 1,130 pages
    site-wide worsened by 0.5 or more, 629 improved. A first draft of the
...
12. **The lever names the kind of work, not the amount.** The `lever` column
    ...
    their em dashes were removed: the cleanup took 6.5 points off one and
