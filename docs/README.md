...
| Pinterest pins (+ new boards) | `generate-pinterest.py`, `generate-id-pins.py`, `generate-es-pins.py`, `generate-vertical-text-pins.py` (skin: `generate-site-art.py`); CSV: `pinterest_csv.py` + `build_pinterest_upload.py` | [`pinterest-pin-generation.md`](./pinterest-pin-generation.md) (board conventions) + [`pinterest-csv-format.md`](./pinterest-csv-format.md) | per batch |
...
6. ~~**One Pinterest board is off-system.**~~ **Closed** — the Spanish `/es/`
   board was migrated out of the orphaned top-level `pinterest-kit/` onto the
   shared pipeline (`scripts/generate-es-pins.py`, `assets/pinterest/es/`,
   `data/es_pinterest_pins[_upload].csv`). All Pinterest boards are now on-system.
