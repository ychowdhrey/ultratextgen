...
| **3. Review** | **done 2026-09-26** | the shadow window was read over 212 merged PRs and 4,599 changed pages. Verdict: **nothing promoted.** `model-leakage` never fired (0 introduced; its one standing hit was a false positive), and `seo-preservation` fired 361 times at roughly **93% false positives** across five distinct classes. Full readout below. |
| **4. Enforce the deterministic rules** | **blocked, re-dated to 2026-10-24** | stage 3 disqualified both candidates on their own decision rule. `model-leakage` has now been repaired twice and needs a window in which it can actually fire. `seo-preservation` needs the five false-positive classes below closed; `concrete-fact-lost` is the one sub-rule worth promoting first (15 of 17 findings were real losses) and is blocked only by its own 2-finding artifact. Promotion stays per-rule via `--enforce <rule,…>`. |
...
| 2026-09-02 | **`specificityDeficit`: the fact vocabulary widened, and part of it is now harvested.** Game names come from `js/gamename/game-rules.js` (every `label`, stripped of its field qualifier, minus anything the platform rule already matches so "PUBG Mobile" is one fact) plus an `EXTRA_GAMES` list for games with no RULES row yet (Forza Horizon, BGMI, Arena of Valor, …). New kinds: `organisation` (Google, Microsoft, Samsung, Unicode Consortium, Emojipedia, Riot Games, …), `emoji-font` (Twemoji, Noto Color Emoji, Segoe UI Emoji, …), `emoji-version` ("Emoji 16.0"), `os-version` ("iOS 18.4"), `date` ("January 1, 2026", "14 July 2026", "March 2024" — three distinct facts, where the bare year rule credited one), `duration` ("1 day to 3 years"), `figure` ("32,768"), `percentage` ("100% plain"), `quantity` ("5-10 comments per day", "10+ reactions"); `limit` now reads "4,096 characters" as one limit instead of "096 characters". Fourteen platform names added (Twitter on 63 English pages, Reddit 9, Twitch 7, Slack 5, …). Deliberately excluded because each is an ordinary word or an emoji name somewhere on this site: Apple, Meta, Word, Signal, Kick, Notion, Stories, Explore. **Site-wide effect measured before landing, against the baseline main carried after its own em-dash template fix: 629 pages improve, 2,870 unchanged, 1,130 worsen by 0.5 or more (largest +11.8, `library/devil-emoji`), site median 10.0 → 10.5; on the two gated sections, `updates/lienquan-mobile-name-penalty-update` 10.9 → 3.3 and `unicode-18-most-anticipated-emoji` 8.5 → 2.2, while three untouched guides crossed out of PASS because the English guide median rose from 6.6 to 7.8 facts per 1,000 words.** A first draft also counted the site's own units ("3 styles", "12 symbols") and was narrowed to engagement counts, since every guide here says "5 fonts" somewhere. Baseline regenerated in the same change. Five assertions added (`test:editorial-footprint` now 77), including the harvest, the de-duplication against the platform rule, and the ambiguity guard. | vocabulary gap; user decision to keep adding factual phrases |
...
| 2026-09-26 | **Shadow-mode readout: nothing promoted.** Read over 212 PRs merged since 2026-08-26 and the 4,599 HTML pages they changed. `model-leakage` **never fired** — 0 introduced findings across the whole window — and its only standing hit anywhere on the site was a false positive, so it is disqualified twice over by its own rule ("never fired → do not promote on silence"; "any false positive → fix and stay in shadow"). `seo-preservation` fired **361 errors** of which roughly **24 were real**: 298 `title-changed`/`h1-changed` from the tone-of-voice rewrites the standard *requires*, 19 `internal-link-lost` on pages still reachable elsewhere, 7 on links whose target 404s (removing a broken link reads as a loss), 2 `hreflang-lost` where a placeholder homepage claim was correctly replaced by a real EN parent built 2026-09-25, and **9 `protected-term-lost` where correct localisation was flagged as a loss** — the German currency page replaced the English loanword "Codepoint" with "Codepunkt" and the gate reported the protected term removed. The real findings are worth keeping: **`concrete-fact-lost` was right 15 times out of 17**, catching "Unicode 1.1" and "Unicode 6.2" dropped from four locale currency pages and a "40 characters" limit dropped from two printables, plus 2 genuinely orphaned links under `ko/`. **No drift**: 4,565 → 4,715 pages scored, p50 10.2 → 10.1, p90 25.4 → 26.3, p99 37.6 → 38.7, and the similarity ceiling held exactly — max Jaccard 0.661 against a recorded 0.67, with zero pages above it; the pair count 878 → 1,829 is 150 more pages in the same cohorts, not tighter duplication. | scheduled review, decision rule applied |
| 2026-09-26 | **`EFR-F-004` narrowed: `reach out` dropped from the alternation.** "If you have suggestions or ideas, feel free to reach out." on `about/index.html` was this rule's only hit anywhere on the site, and it is ordinary reader-facing contact copy on the one page type where it belongs — not scaffolding addressed to a commissioner. `let me know` is kept, which has no reader-facing reading. The entry also shipped claiming `corpusFrequency: 0` while the corpus already held this occurrence; corrected. Had the rule been promoted on 2026-08-26, the next PR touching that page's prose would have been blocked on a legitimate sentence. | false positive found by the readout |
| 2026-09-26 | **`EFR-F-004`: two of its five alternatives had been unreachable since the day it shipped.** `Certainly[,!]` and `In conclusion,` sat inside `\b(?:…)\b`, and a trailing `\b` cannot assert after `,` or `!` — both are non-word characters, as is the space that follows. Neither could ever match. This is the repo's most-repeated failure — a check that reports nothing is indistinguishable from a check that passes — sitting inside the rule that was up for promotion today, and it was found by *probing* the rule after narrowing it, not by reading it. The word-ending alternatives keep the trailing `\b`; the two punctuation-ending ones now sit outside it. Repairing them fires on **0** of 4,725 live pages, so the fix introduces no new false positive. Two assertions added to `npm run test:editorial-footprint` (now **79**), both verified failing against the old pattern. | dead rule, found by probe |
---
