...
(() => {
  ...
  // Icons live in share-core.js, this file's only source for them. Read
  // lazily through a getter object so load order between the two cannot
  // matter: an empty string renders a button with its label and no glyph,
  // never a crash.
  const ICONS = {
    get share() { return (UTG.icons && UTG.icons.share) || ""; },
    get copy()  { return (UTG.icons && UTG.icons.copy)  || ""; },
    get image() { return (UTG.icons && UTG.icons.image) || ""; }
  };
...
const decorations = window.UTG_DECORATIONS
  ...
  // Saved styles (return-driving mechanic). The store itself moved to
  // /js/saved/saved-items.js (2026-09-05), which is typed — a saved font
  // style and a saved symbol are both records now — and shared with the
  // library and symbol pages. This file only ever wants the style-shaped
  // view of it, which is what savedStyleNames() returns.
  function savedStyleNames() {
    return UTG.saved ? UTG.saved.all("style").map((r) => r.value) : [];
  }
  ...
  function isSaved(name) {
    return !!UTG.saved && UTG.saved.has("style", name);
  }

  function toggleSaved(name) {
    // The registry guard stays: this page's unit of content is a style, and
  ...
  /* ===================
     RESULT SHARING
     =================== */
  // Moved to /js/share/share-core.js (2026-09-05) so that every surface
  // offering Copy can offer Share without loading this 120KB file. That
  // module owns buildShareUrl / shareCreation / shareCreationAsImage, the
  // button factories, the ?style= reader and both delegated click handlers,
  // and publishes them on the same window.UltraTextGen namespace this file
  // already used — so every call site below is unchanged. Pages that load
  // script.js must also load share-core.js; scripts/check-share-save-tags.js
  // gates that.
      ...
      <div class="style-actions-stack">
        ...
        <div class="result-share-row">
          <button class="share-result-btn" data-style="${safeName}" data-share-id="${safeAttr(shareId)}" type="button" ${!fullText ? "disabled" : ""} title="${safeAttr(shareTitle)}" aria-label="${safeAttr(shareAria)}">
            ${ICONS.share}<span class="share-result-label">${escapeHtml(shareLabel)}</span>
          </button>
          <button class="share-image-btn" data-share-id="${safeAttr(shareId)}" data-share-name="${safeName}" type="button" ${!fullText ? "disabled" : ""} title="${safeAttr(uiText("shareResult.imageTitle", "Share as an image"))}" aria-label="${safeAttr(uiText("shareResult.imageAriaLabel", "Share {style} result as an image").replace("{style}", name))}">${ICONS.image}</button>
        </div>
      </div>
    `;
  ...
  function ensureSavedSection() {
    ...
    if (clearBtn) {
      clearBtn.addEventListener("click", () => {
        if (UTG.saved) UTG.saved.clear("style");
        renderSavedStyles();
        renderResults();
      });
    }
  ...
  function renderSavedStyles() {
    ...
    const names = savedStyleNames();
    const valid = names.filter((name) => stylesRegistry[name]);

    // Drop any saved names that no longer exist in the registry.
    if (valid.length !== names.length && UTG.saved) {
      names.forEach((name) => {
        if (!stylesRegistry[name]) UTG.saved.toggle({ type: "style", value: name, label: name });
      });
    }
...
// The delegated .share-result-btn / .share-image-btn click handlers moved to
// /js/share/share-core.js with the rest of the share core. They read their
// creation state from the card and from data-share-* on the button, so they
// keep working for the cards this file builds.
