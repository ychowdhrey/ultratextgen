 ...
 * scores each FAQ by answer length and cross-page uniqueness,
 * selects the top 10, builds FAQPage JSON-LD, and injects it before </head>.
 *
 * Two-pass approach:
 *   Pass 1 — Extract FAQs from ALL pages and build a global bigram index.
 *   Pass 2 — For each page, score FAQs against both in-page and cross-page
 *            duplicates, select the top MAX_FAQ, and inject.
 *
 * Scoring formula (per FAQ item):
 *   lengthScore      = min(answerPlainText.length / 400, 1)              0 → 1
 *   inPageUnique     = 1 − max Jaccard overlap with in-page selected Qs  0 → 1
 *   crossPageUnique  = 1 − max Jaccard overlap with all other pages' Qs  0 → 1
 *   totalScore       = 0.35 × lengthScore
 *                    + 0.35 × inPageUnique
 *                    + 0.30 × crossPageUnique
 *
...
/**
 * Find the maximum Jaccard similarity between a bigram set and
 * an array of other bigram sets.  Returns 0 if the array is empty.
 */
function maxSimilarity(qBigrams, otherBigramSets) {
  let max = 0;
  for (const other of otherBigramSets) {
    const sim = jaccardSimilarity(qBigrams, other);
    if (sim > max) max = sim;
  }
  return max;
}
...
/**
 * Score and rank FAQ items with cross-page awareness.
 *
 * @param {Array}  items              - FAQ items from this page
 * @param {number} limit              - max items to select (MAX_FAQ)
 * @param {Array}  crossPageBigrams   - bigram Sets from questions on OTHER pages
 * @param {string} filePath           - current file (for logging)
 *
 * Each item is scored on three axes:
 *   1. Length score (35%)        - longer answers are more valuable.
 *   2. In-page uniqueness (35%) - penalizes near-duplicates within this page.
 *   3. Cross-page uniqueness (30%) - penalizes generic questions that appear
 *      on many pages (e.g. "Is UltraTextGen free?", "What is UltraTextGen?").
 *
 * Uses greedy selection so in-page uniqueness is evaluated relative to
 * the items already picked for this page.
 */
function rankFaqItems(items, limit, crossPageBigrams, filePath) {
  ...
  while (selected.length < limit && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      // -- In-page uniqueness (0 -> 1) --
      // Compared against items already selected for THIS page
      const inPageOverlap = maxSimilarity(
        item.qBigrams,
        selected.map((s) => s.qBigrams)
      );
      const inPageUnique = 1 - inPageOverlap;

      // -- Cross-page uniqueness (0 -> 1) --
      // Compared against questions from ALL OTHER pages
      const crossPageOverlap = maxSimilarity(item.qBigrams, crossPageBigrams);
      const crossPageUnique = 1 - crossPageOverlap;
...
`);
  selected.forEach((s, i) => {
    ...
    const cpOverlap = maxSimilarity(s.qBigrams, crossPageBigrams);
    const tag = cpOverlap >= 0.5 ? " ⚠ shared" : " ✦ unique";
    console.log(
      `       ${i + 1}. [${len} chars]${tag} ${s.question.slice(0, 72)}${s.question.length > 72 ? "..." : ""}`
    );
  });
...
function main() {
  ...
  /* ── Pass 1: extract FAQs from every page, build global index ── */
  console.log("── Pass 1: Extracting FAQs from all pages...\n");

  const pageMap = new Map(); // filePath -> { html, items, itemBigrams[] }

  for (const file of files) {
    const html = fs.readFileSync(file, "utf-8");
    const items = extractFaqItems(html);
    const itemBigrams = items.map((item) => bigrams(item.question));
    pageMap.set(file, { html, items, itemBigrams });

    if (items.length > 0) {
      console.log(`   📄 ${path.relative(process.cwd(), file)} — ${items.length} FAQs`);
    }
  }

  // Count total and shared questions across pages
  const allBigrams = [];
  for (const { itemBigrams } of pageMap.values()) {
    allBigrams.push(...itemBigrams);
  }
  console.log(`\n   🌐 Total FAQ items across all pages: ${allBigrams.length}\n`);

  /* ���─ Pass 2: score, rank, and inject per page ── */
  console.log("── Pass 2: Scoring with cross-page awareness...\n");

  for (const [file, { html, items, itemBigrams }] of pageMap) {
    const relPath = path.relative(process.cwd(), file);

    if (items.length === 0) {
      console.log(`⏭  ${relPath} — no FAQ items, skipping.`);
      continue;
    }

    // Build cross-page bigrams: all questions from OTHER pages
    const crossPageBigrams = [];
    for (const [otherFile, otherData] of pageMap) {
      if (otherFile === file) continue;
      crossPageBigrams.push(...otherData.itemBigrams);
    }

    console.log(`📝 ${relPath} (${items.length} FAQs, ${crossPageBigrams.length} cross-page comparisons)`);

    // Score, rank, and select the top FAQs
    const topItems = rankFaqItems(items, MAX_FAQ, crossPageBigrams, file);

    const output = injectJsonLd(html, topItems);
    fs.writeFileSync(file, output, "utf-8");
    console.log(`✅ ${relPath} — injected ${topItems.length}/${items.length} FAQ items.\n`);
  }
